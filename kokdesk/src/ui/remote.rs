...
impl Handler {
    ...
    fn get_video_fps(&mut self) -> i32{
        return self.lc.read().unwrap().video_fps.clone();
    }
    ...
    fn save_video_fps(&mut self, value: i32) {
        let msg = self.lc.write().unwrap().save_video_fps(value);
    ...
    if handler.is_port_forward() {
        if handler.is_rdp() {
            ...
            if handler.args.len() != 3
                ...
                || port <= 0
            {
                handler.on_error("Invalid arguments, usage:<br><br> kokdesk --port-forward remote-id listen-port remote-host remote-port");
            }
