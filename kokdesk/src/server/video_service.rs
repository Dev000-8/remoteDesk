...
lazy_static::lazy_static! {
    ...
    static ref VIDEO_FPS: Arc<Mutex<HashMap<i32, i32>>> = Default::default();
...
pub fn update_video_fps(id: i32, q:Option<i32>) {
    match q {
        Some(q) => {
            if q > 0 {
                VIDEO_FPS.lock().unwrap().insert(id, q);
            } else {
                VIDEO_FPS.lock().unwrap().remove(&id);
            }
        }
        None => {
            VIDEO_FPS.lock().unwrap().remove(&id);
