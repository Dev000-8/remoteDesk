...
static IPC_FILE: &'static str = "/tmp/KOKDesk/pynput_service";

fn start_pynput_service(rx: mpsc::Receiver<(PyMsg, bool)>) {
    let mut py = "./pynput_service.py".to_owned();
    if !std::path::Path::new(&py).exists() {
        py = "/usr/share/kokdesk/files/pynput_service.py".to_owned();
        if !std::path::Path::new(&py).exists() {
            py = "/usr/lib/kokdesk/pynput_service.py".to_owned();
            if !std::path::Path::new(&py).exists() {
